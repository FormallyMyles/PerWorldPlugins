		...
		if (sender.isOp() || sender.hasPermission("pwp.admin")) {
			if (args.length == 0) {
				...
				if (args[0].equalsIgnoreCase("version")) {
					sender.sendMessage(ChatColor.GREEN + "Running v" + Plugin.instance.getDescription().getVersion()
							+ " of PerWorldPlugins.");
				}
			}
		} else {
			sender.sendMessage(ChatColor.RED
					+ "You do not have permission to use this command. (Permission node: pwp.admin)");
		}
		return true;
	}
